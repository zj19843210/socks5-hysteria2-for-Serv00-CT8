...
# 提示用户是否启用 gRPC 端口的 SSL/TLS 加密
printf "是否启用针对 gRPC 端口的 SSL/TLS 加密 (--tls)，需要请按 [Y]，默认是不需要，不理解的用户可回车跳过: "
read -r NZ_GRPC_PROXY

# 检查用户输入是否为 'Y'，如果是，则设置 ARGS 变量
if echo "${NZ_GRPC_PROXY}" | grep -qiw 'Y'; then
    ARGS='--tls'
else
    ARGS=''  # 默认不启用 SSL/TLS
fi
